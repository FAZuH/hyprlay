...
Each scope maps to one crate or front. If no scope fits, omit the scope. Do not create other scopes.
...
The root package (`Cargo.toml` + `src/`) contains four fronts. Each front maps to one scope as shown above. `crates/hyprlay-core` maps to `core`.
